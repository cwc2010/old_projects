...
<head>
  ...
  <title>FLIP Water – Step 3 (Particles Init)</title>
...
/*
  ...
  This file is at Step 3:
    - We keep the MAC grid from Step 2
    - We initialize particles and assign a fixed particleVolume so mass (area) is conserved
    - No transfers or pressure solve yet (no simulation), but we preview the seeded particles
*/
...
// Face-centered grid velocities and their "previous" copies (for FLIP delta later)
let xFaceVelocity = null;      // (gridWidth+1) * gridHeight
let yFaceVelocity = null;      // gridWidth * (gridHeight+1)
let xFaceVelocityPrev = null;  // same as xFaceVelocity
let yFaceVelocityPrev = null;  // same as yFaceVelocity

// Mass accumulators for P2G normalization (used in later steps)
let xFaceMass = null;
let yFaceMass = null;

// Cell-centered pressure (later)
let pressure = null;

// Cell occupancy/density (used later for marching squares)
...
/* ------------------------------
   Step 3: Particles & volumes
   ------------------------------
   We seed a fixed number of particles in a region (bottom portion of domain).
   Each particle represents a constant area 'particleVolume' (2D analog of volume),
   computed as cellArea / particlesPerCellInit. We never create or destroy particles –
   this ensures strict mass conservation across the whole simulation.
*/
let particlesPerCellInit = 6; // recommended 4–8; we'll auto-choose per device below
let particleVolume = 0;       // area represented by one particle
let totalVolume = 0;          // total area represented by all particles
let particlesCount = 0;       // number of particles (fixed after init)
...
// Heuristic to pick particles-per-cell based on screen/device
function chooseParticlesPerCell() {
  // Slightly fewer particles on small screens or high-DPR devices
...
function initializeParticles() {
  ...
  // Fill the bottom portion of the grid (like a tank half-full of water)
  const fillFractionY = 0.55; // ~55% of height from bottom
  ...
  // Allocate typed arrays for particles
  ...
  // Particle "volume" (2D area) is constant: cellArea / particlesPerCellInit
  const cellArea = cellSize * cellSize;
  particleVolume = cellArea / particlesPerCellInit;
  totalVolume = particleVolume * particlesCount;

  // Seed particles with slight random jitter inside each cell
  let p = 0;
  for (let j = startRow; j < gridHeight; j++) {
    ...
    for (let i = 0; i < gridWidth; i++) {
      ...
      for (let k = 0; k < particlesPerCellInit; k++) {
        // Low-discrepancy-ish stratified jitter within the cell
        const u = ((k + 0.5) % particlesPerCellInit) / particlesPerCellInit;
        const v = ((k * 3 + 0.5) % particlesPerCellInit) / particlesPerCellInit;
        // Mix in small randomness to break up regular patterns
        const rx = (u + (Math.random() - 0.5) * 0.35) * cellSize;
        const ry = (v + (Math.random() - 0.5) * 0.35) * cellSize;

        particlePosX[p] = x0 + Math.min(Math.max(rx, 0.25), cellSize - 0.25);
        particlePosY[p] = y0 + Math.min(Math.max(ry, 0.25), cellSize - 0.25);
        particleVelX[p] = 0;
        particleVelY[p] = 0;
        p++;
      }
...
/* ------------------------------
   Canvas sizing & grid rebuild
   ------------------------------ */

// Compute a cell size that keeps ~100–160 cells across (desktop), fewer on mobile
function chooseCellSize() {
  ...
  // Allow a wider range so we actually hit ~160 across on large screens
...
function rebuildGrid() {
  ...
  // Initialize particles now that grid/cellSize are known
  initializeParticles();
...
function resizeCanvas() {
  ...
  rebuildGrid();   // also (re)initializes particles

  drawPlaceholder(0); // draw once immediately after resizing
}
...
function startLoop() {
  ...
  const tick = (now) => {
    ...
    // Step 3: still no simulation, but preview the seeded particles
...
/* ------------------------------
   Placeholder rendering (Step 3)
   ------------------------------ */

const showParticlesPreview = true;
...
function drawPlaceholder(deltaTime) {
  ...
  // Preview the seeded particles as small dots (debug-only, capped for perf)
  if (showParticlesPreview && particlesCount > 0) {
    drawParticlesPreview();
  }

  // Title/info badge (top-left). UI is top-right so it won't overlap.
  const badgeW = 620;
  const badgeH = 142;
  ctx.save();
  ...
  ctx.fillText("Step 3: Particles initialized & constant volume assigned", 36, 86);

  // Grid + particles summary line
  ctx.fillStyle = "rgba(195,215,255,0.9)";
  ctx.font = "12px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto";
  ctx.fillText(
    `Grid: ${gridWidth} × ${gridHeight} • Cell: ${cellSize}px • Particles: ${particlesCount} (ppc: ${particlesPerCellInit}) • ParticleVolume: ${particleVolume.toFixed(2)} • TotalVolume: ${totalVolume.toFixed(0)}`,
    36,
    106
  );
  ...
  ctx.font = "12px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto";
  ctx.fillText(
    "Note: no physics yet. Particles are seeded and conserved; next steps will transfer velocities and solve pressure.",
    36,
    126
  );
  ctx.restore();
...
function drawParticlesPreview() {
  const dotSize = Math.max(1, Math.floor(cellSize * 0.25));
  // Cap draws to ~20k points for performance; stride skips evenly
  const maxDots = 20000;
...
resetBtn.addEventListener("click", () => {
  // Re-seed particles (preserving current grid/cellSize)
  initializeParticles();
...
Apply changes from step 4 (Main loop (per timestep)). Code runs as expected.

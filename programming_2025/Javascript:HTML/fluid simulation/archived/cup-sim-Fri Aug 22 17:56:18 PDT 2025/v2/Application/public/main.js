...
/*
  FLIP (Fluid-Implicit Particle) Simulation Explained
  ----------------------------------------------------
  This simulation uses the FLIP method, which combines the strengths of both
  particles and grids to create realistic liquid effects.

  Core Idea:
  - Particles are the "memory" of the fluid. They carry properties like velocity
    and are free to move around, which makes them great for tracking the fluid's
    motion and surface.
  - A Grid is a fixed background structure. We use it to calculate forces like
    pressure, which is very difficult to do with particles alone. The grid ensures
    the fluid behaves realistically—for example, by not being compressible (like water).

  The FLIP Main Loop (for each time step):
  1. Particle to Grid (P2G): We take the velocity from each particle and transfer it
     to the nearby cells of the grid. This gives us a velocity field on the grid.
  2. Pressure Projection: This is the magic step that makes the fluid incompressible.
     We calculate the pressure needed to make the velocity on the grid "divergence-free"
     (meaning no fluid is created or destroyed in any cell). This stops the fluid from
     bunching up or spreading out unnaturally.
  3. Grid to Particle (G2P): We then update each particle's velocity based on the *change*
     that happened on the grid due to pressure. We calculate the difference
     (grid velocity after pressure - grid velocity before pressure) and add that
     difference to the particle's velocity.
  4. Apply Gravity & Advect: We apply external forces like gravity directly to each
     particle's velocity, and then move the particles according to their final new velocities.
*/
...
const fixedDeltaTime = 1 / 60; // seconds
...
let xFaceVelocity = null;
let yFaceVelocity = null;
let xFaceVelocityPrev = null;
let yFaceVelocityPrev = null;
...
let pressure = null;
let pressureNext = null;
let divergence = null;
let isFluidCell = null;
...
/* ------------------------------
   ...
   ------------------------------ */
let particlesPerCellInit = 6;
let particleVolume = 0;
let totalVolume = 0;
...
function chooseJacobiIterations() {
  ...
  if (cells > 36000 || dpr > 2) return 30;
  if (cells > 24000) return 40;
...
/* ------------------------------
   Particle → Grid (P2G) transfer
   ------------------------------ */
function p2gTransferAndOccupancy() {
  const particleMass = particleVolume; // ρ=1, so mass == volume in 2D
  const gw = gridWidth, gh = gridHeight;

  for (let p = 0; p < particlesCount; p++) {
    const x = particlePosX[p];
    const y = particlePosY[p];
    const velocityX = particleVelX[p];
    const velocityY = particleVelY[p];
    ...
    // Y-face (horizontal faces at i+0.5, j)
    {
      ...
      if (i0 >= 0 && i0 < gw && j0 >= 0 && j0 <= gh) {
        ...
        yFaceVelocity[idx] += velocityY * w00 * particleMass;
        yFaceMass[idx] += w00 * particleMass;
      }
      if (i1 >= 0 && i1 < gw && j0 >= 0 && j0 <= gh) {
        const idx = yFaceIndex(i1, j0);
        yFaceVelocity[idx] += velocityY * w10 * particleMass;
        yFaceMass[idx] += w10 * particleMass;
      }
      if (i0 >= 0 && i0 < gw && j1 >= 0 && j1 <= gh) {
        const idx = yFaceIndex(i0, j1);
        yFaceVelocity[idx] += velocityY * w01 * particleMass;
        yFaceMass[idx] += w01 * particleMass;
      }
      if (i1 >= 0 && i1 < gw && j1 >= 0 && j1 <= gh) {
        const idx = yFaceIndex(i1, j1);
        yFaceVelocity[idx] += velocityY * w11 * particleMass;
        yFaceMass[idx] += w11 * particleMass;
      }
    }
  }

  // Normalize faces by accumulated mass to get the momentum-preserving average
...
/* ------------------------------
   ...
   ------------------------------ */
function integrateParticles(deltaTime) {
  ...
  for (let p = 0; p < particlesCount; p++) {
    // Semi-implicit Euler
    particlePosX[p] += particleVelX[p] * deltaTime;
    particlePosY[p] += particleVelY[p] * deltaTime;

    // Collide with domain bounds
...
function simulateStep(deltaTime) {
  ...
  // 3) Save grid velocities BEFORE pressure solve for FLIP delta
  ...
  // 5) G2P (FLIP with small PIC blend)
  for (let p = 0; p < particlesCount; p++) {
    const x = particlePosX[p];
    const y = particlePosY[p];
    ...
    const deltaVelX = newGrid.x - oldGrid.x;
    const deltaVelY = newGrid.y - oldGrid.y;

    const velocityFlipX = particleVelX[p] + deltaVelX;
    const velocityFlipY = particleVelY[p] + deltaVelY;

    // PIC-Flip blend
    particleVelX[p] = velocityFlipX * (1 - picBlend) + newGrid.x * picBlend;
    particleVelY[p] = velocityFlipY * (1 - picBlend) + newGrid.y * picBlend;
  }
  
  // 6) Apply gravity directly to particles
  const accelX = gravity.x * deltaTime;
  const accelY = gravity.y * deltaTime;
  for (let p = 0; p < particlesCount; p++) {
    particleVelX[p] += accelX;
    particleVelY[p] += accelY;
  }

  // 7) Advect particles and handle walls
...
function render() {
  ...
  ctx.fillText("Step 5: P2G details + corrected FLIP force ordering (gravity now affects particles)", 36, 86);
...
function startLoop() {
  ...
  const tick = (now) => {
    ...
    let steps = 0;
    while (timeAccumulator >= fixedDeltaTime && steps < 5) {
      simulateStep(fixedDeltaTime);
...
pauseBtn.addEventListener("click", () => {
  ...
  pauseBtn.setAttribute("aria-pressed", isSimulationPaused ? "true" : "false");
  pauseBtn.textContent = isSimulationPaused ? "Resume" : "Pause";
  setStatus(isSimulationPaused ? "Paused" : "Running");
});

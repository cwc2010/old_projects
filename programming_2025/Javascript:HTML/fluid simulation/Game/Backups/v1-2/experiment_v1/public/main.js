...
const grid_size = 500;                      // number of cells per side
...
/*──────────────────── GPU.js Setup ─────────────────────────────*/
// Dynamically load gpu.js if it does not yet exist
if (typeof GPU === 'undefined') {
  const s = document.createElement('script');
  s.src   = 'https://cdn.jsdelivr.net/npm/gpu.js@latest/dist/gpu-browser.min.js';
  document.head.appendChild(s);
}

let gpuSupported = false;
let gpu;        // will hold the GPU instance (if any)

// we’ll try to create the context as soon as gpu.js is ready
function initGPU() {
  if (typeof GPU === 'undefined' || gpuSupported) return;   // not ready or already done
  try {
    try {
      gpu = new GPU();                        // let gpu.js decide best backend
    } catch (e) {
      gpu = new GPU.GPU()
    }
    gpuSupported = gpu.mode !== 'cpu';      // true only if we really got WebGL/WebGPU
    console.log('[GPU] initialised – hw-accel:', gpuSupported);
  } catch (e) {
    console.warn('[GPU] disabled (reason →', e, ')');
  }
}
initGPU();
...
/*─────────────── Helpers for 1-D ⇆ 2-D conversion (gpu.js) ───────────────*/
function to2D(flat) {            // Float32Array  → nested typed array
  const rows = new Array(grid_size);
  for (let y = 0; y < grid_size; y++) {
    rows[y] = flat.subarray(y * grid_size, (y + 1) * grid_size);
  }
  return rows;
}
function from2D(src2D, destFlat) {   // nested → Float32Array (in-place)
  for (let y = 0; y < grid_size; y++) destFlat.set(src2D[y], y * grid_size);
}

/*────────────────── Compile GPU kernels (only once) ──────────────────*/
function initGPUKernels() {
  if (!gpuSupported || gpu.diffuse) return;  // either not supported or already done

  const sz = [grid_size, grid_size];

  // One Gauss-Seidel diffusion pass
  gpu.diffuse = gpu.createKernel(
    function (prev, coeff, a, N) {
      const x = this.thread.x, y = this.thread.y;
      if (x === 0 || y === 0 || x === N - 1 || y === N - 1) return prev[y][x];
      return (prev[y][x] + coeff * (
                prev[y][x - 1] + prev[y][x + 1] +
                prev[y - 1][x] + prev[y + 1][x]
              )) / a;
    }
  ).setOutput(sz);
  // keep data on GPU between calls
  gpu.diffuse
    .setPipeline(true)
    .setImmutable(true);
  ...
  gpu.advect = gpu.createKernel(
    function (prev, velX, velY, dt, N) {
      const x = this.thread.x;
      const y = this.thread.y;
      if (x === 0 || y === 0 || x === N - 1 || y === N - 1) return prev[y][x];

      // back-trace
      let bx = x - dt * velX[y][x];
      let by = y - dt * velY[y][x];
      bx = Math.min(Math.max(bx, 0.5), N - 1.5);
      by = Math.min(Math.max(by, 0.5), N - 1.5);

      const i0 = Math.floor(bx), j0 = Math.floor(by);
      const i1 = i0 + 1,         j1 = j0 + 1;
      const sx = bx - i0,        sy = by - j0;

      const f00 = prev[j0][i0], f10 = prev[j0][i1];
      const f01 = prev[j1][i0], f11 = prev[j1][i1];

      const ix0 = f00 * (1 - sx) + f10 * sx;
      const ix1 = f01 * (1 - sx) + f11 * sx;
  ...
  // simple damping / decay
  gpu.decay = gpu.createKernel(
    function (field, factor) {
      return field[this.thread.y][this.thread.x] / factor;
    }
  ).setOutput(sz);
  gpu.decay
    .setPipeline(true)
  ...
  // wrap a flat Float32Array into a GPU texture
  /*gpu.wrapField = gpu.createKernel(function(f) {
    return f[this.thread.y][this.thread.x];
  })
  .setOutput(sz)
  .setOutput(sz)
  .setConstants({ N: grid_size })
  .setPipeline(true)
  .setImmutable(true);*/
}
initGPUKernels();
...
const player = {
  ...
  bulletRadius: 10,     // how big the influence zone is
  ...
  fireCooldown: 500,   // seconds between shots
  bulletKnockback: 4, // multiplier to knock player back when shooting
...
function simulateStep() {
  ...
  /*──────────────────── Fluid solve (GPU ⇆ CPU) ────────────────────*/
  initGPU();         // (re-)try init if script finished loading
  initGPUKernels();  // same for kernels

  if (gpuSupported) {                 /* ─── GPU PATH ─── */
    let vX   = to2D(velocityXField);
    let vY   = to2D(velocityYField);
    let dens = to2D(densityField);

    /* 5) & 6) velocity diffusion + advection */
    const aVel = 1 + 4 * viscosity * time_step;
    for (let i = 0; i < solver_iterations; i++) {
      vX = gpu.diffuse(vX, viscosity * time_step, aVel, grid_size);
      vY = gpu.diffuse(vY, viscosity * time_step, aVel, grid_size);
    }
    vX = gpu.advect(vX, vX, vY, time_step, grid_size);
    vY = gpu.advect(vY, vX, vY, time_step, grid_size);

    /* 7) density diffusion + advection */
    // use a distinct denominator for density
    const aDensity = 1 + 4 * density_diffusion * time_step;
    for (let i = 0; i < solver_iterations; i++) {
      dens = gpu.diffuse(dens, density_diffusion * time_step, aDensity, grid_size);
    }
    dens = gpu.advect(dens, vX, vY, time_step, grid_size);

    /* damping */
    vX   = gpu.decay(vX,   1.01);
    vY   = gpu.decay(vY,   1.01);
    dens = gpu.decay(dens, 1.1);

    // only one back-transfer per frame
    from2D(vX.toArray(),   velocityXField);
    from2D(vY.toArray(),   velocityYField);
    from2D(dens.toArray(), densityField);
    vX.delete();
    vY.delete();
    dens.delete();

    /* keep incompressibility (CPU Poisson solver) */
    ...
    // damping
    for (let i = 0; i < velocityXField.length; i++) velocityXField[i] /= 1.01;
    for (let i = 0; i < velocityYField.length; i++) velocityYField[i] /= 1.01;
    for (let i = 0; i < densityField.length;  i++) densityField[i]  /= 1.1;
  }

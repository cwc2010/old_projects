...
You are in the proccess of making step 5 (Particle → Grid transfer (P2G) details). The frames appear to run at 60 frames per second, and the rendering works as expected, but the behavior of the water does not appear to be stable. The particles do not fall due to gravity, but they do stop when hitting the walls. I ran the simulation and selected certain frames to attach as images.

Provide your outputs with the file names and code block. Tell me the expected output when you are done. If revising files, provide the ENTIRE file instead of telling me what to edit. 